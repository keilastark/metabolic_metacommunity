...
@showprogress for b in eachindex(warming_V)
    ...
    @showprogress for rep = 1:reps
                   ...
                   global Model_df
                    ...
                    @showprogress for gen = 1:Tmax
                        ...
                        if d_temp_function == "unimodal"
                            disp_temp = d * exp(0.65/8.62e-05 * (1/293.15 - 1/(x+ 273.15))) * (1 / (1 + (0.65/(2 - 0.65)) * exp(2/8.62e-05 * (1/(topt + 273.15) - 1/(x + 273.15)))))
                        elseif d_temp_function == "monotonic"
                            disp_temp = d * exp(-0.65/8.62e-05 * (1/293.15 - 1/(x+273.15)))
        ...
        CSV.write("./outputs/alpha_disp_outputfile2_$(warming_write)_$(rep).csv", Model_df)
    end
end
